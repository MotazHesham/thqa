...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.buildingDocument.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.building-documents.update", [$buildingDocument->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('building') ? 'is-invalid' : '' }}" name="building_id" id="building_id">
                    @foreach($buildings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('building_id') ? old('building_id') : $buildingDocument->building->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('file_num') ? 'is-invalid' : '' }}" type="text" name="file_num" id="file_num" value="{{ old('file_num', $buildingDocument->file_num) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('file_name') ? 'is-invalid' : '' }}" type="text" name="file_name" id="file_name" value="{{ old('file_name', $buildingDocument->file_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('file_type') ? 'is-invalid' : '' }}" type="text" name="file_type" id="file_type" value="{{ old('file_type', $buildingDocument->file_type) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('file_date') ? 'is-invalid' : '' }}" type="text" name="file_date" id="file_date" value="{{ old('file_date', $buildingDocument->file_date) }}">
